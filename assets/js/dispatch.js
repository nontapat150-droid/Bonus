...
window.downloadTemplate = function(type) {
    ...
    if (type === 'office') {
        ...
        ws_data = [
            ["รหัส", "ชื่อลูกค้า", "เบอร์โทร", "ที่อยู่", "ละติจูด", "ลองจิจูด", "วันที่", "แพ็กเกจ", "หมายเหตุ"],
            ["AC-12345", "สมชาย ใจดี", "0812345678", "123 ม.1 ต.ทดสอบ อ.เมือง จ.กรุงเทพฯ", "13.123456", "100.123456", "2023-12-31", "Fiber 1000/500", "ทดสอบระบบ"]
        ];
    } else if (type === 'ma') {
        fileName = 'template_ma.xlsx';
        ws_data = [
            ["เวลา", "NON", "ชื่อลูกค้า", "เบอร์โทร", "พื้นที่", "ตำบล", "อำเภอ", "ที่อยู่", "อาการ", "ทีม", "หมายเหตุ", "วันที่"],
            ["09:00", "NON-9999", "สมหญิง น่ารัก", "0898765432", "AIS", "บางรัก", "บางรัก", "456 ม.2", "เน็ตหลุดบ่อย", "ทีม A", "ลูกค้าด่วน", "2023-12-31"]
        ];
    }

    if (typeof XLSX === 'undefined') {
        Swal.fire('ข้อผิดพลาด', 'ไม่พบไลบรารีสำหรับสร้างไฟล์ Excel', 'error');
        return;
    }
